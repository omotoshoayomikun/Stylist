...
function Sidebar(props) {
  ...
  const Details = useSelector((state) => state.auth.userDetails[0])
  ...
  const createLinks = (routes) => {
    return routes.map((prop, key) => {
      if (role == 'Customer' && prop.role == 'Customer') {
        return (
      ...
      if (role == 'BusinessOwner' && prop.role == 'BusinessOwner') {
        return (
          <NavItem key={key} active={activeRoute(prop.layout + prop.path)}>
            <Link href={prop.layout + prop.path}>
              <NavLink
  ...
  return (
    ...
    >
      <Container fluid>
        ...
        <h2 style={{ marginBottom: '-20px' }} className="text-center">STYLIST</h2>
        {/* User */}
        ...
        <Collapse navbar isOpen={collapseOpen}>
          ...
          {/* Navigation */}
          <Nav navbar>
            <NavItem active={activeRoute('/admin/dashboard')}>
            ...
            {createLinks(routes)}
            <NavItem active={activeRoute('/admin/profile')}>
              <Link href="/admin/profile">
                <NavLink
                  href="#pablo"
                  active={activeRoute('/admin/profile')}
                  onClick={closeCollapse}
                >
                  <i className="ni ni-tv-2 text-primary" />
                  Profile
            ...
            <NavItem active={activeRoute('/admin/settings')}>
              <Link href="/admin/settings">
                <NavLink
                  href="#pablo"
